...
func UpdateUserSetting(c *gin.Context) {
	// Read body once so we can bind known fields and still extract plugin extension keys.
	...
	// 构建设置（保留已有 Extensions，避免部分更新擦除插件配置）
	settings := dto.UserSetting{
		...
		Extensions:                       existingSettings.Extensions,
	}
	...
	if err := common.Unmarshal(rawBody, &reqMap); err == nil {
		if settings.Extensions == nil {
			settings.Extensions = make(map[string]json.RawMessage)
		}
		for _, p := range plugin.All() {
			...
			raw, ok := reqMap[desc.JSONKey]
			if !ok {
				continue
			}
			if err := p.OnSettingsUpdate(raw); err != nil {
				common.ApiError(c, err)
				return
			}
			settings.Extensions[desc.JSONKey] = raw
		}
	}
